...
$outputFileName = "C:\Vault Data\orphans.csv"
$logFileName = "C:\vault Data\OrphanSearchLog.txt"
        ...
        ForEach ($vaultfile in $results)
        {
            try
            {
                ...
                if ($parents.Count -eq 0)
                {
                    ...
                    $coFolder = 
                    $dt.Rows.Add($coFile.Name, $coFile._FullPath, $coFile.Version, $cofile.Category, $coFile.State, $coFile.MasterId, $vaultfile.FolderId, $cofile.Manager, $coFile._CheckInDate)
                }
            }
            catch
            {
