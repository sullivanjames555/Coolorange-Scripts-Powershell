...
#########  SCRIPT VARIABLES
$csvFileLocation = "C:\Vault Data\Vault Reports\Orphan Files\2021\orphans 4-16.csv"
$outputFileName = "C:\Vault Data\Vault Reports\Deleted_Orphans$(Get-Date -Format "yyyyMMdd-hhmmss")_Log.csv" 
...
function DeleteFile
{
    ...
    if($deleteFiles)
    {
        try
        {
            ...
            #running without vault restrictions turn on this feature -jps
            $vault.DocumentService.DeleteFileFromFolderUnconditional($masterID, $folderID)
            $result = "Succeeded"
        }
        catch
        {
...
$withoutmanager = @()
$filewithoutmanager = @()
foreach($filemanager in $filetable){

if( $filemanager.Manager -notcontains "203121" -and $filemanager.Manager -notcontains "208458"){$filewithoutmanager += $filemanager}}




$deleteHash = @{}
foreach ($fileRow in $filewithoutmanager)
{
    #Write-Host "$($fileRow."FileName") -- $($fileRow.Date)"
        ...
        foreach($prefix in $90dayDeletePrefixes)
        {
            if($fileRow.FileName.StartsWith($prefix))
            {
                ...
                $dt.Rows.Add($fileRow.FileName, $fileRow.VaultPath, $fileRow.Date,$filerow.Manager, $result)
        ...
        foreach($prefix in $120dayDeletePrefixes)
        {
            if($fileRow.FileName.StartsWith($prefix))
            {
                ...
                $dt.Rows.Add($fileRow.FileName, $fileRow.VaultPath, $fileRow.Date, $filerow.Manager, $result) | Out-Null
            }
        }
