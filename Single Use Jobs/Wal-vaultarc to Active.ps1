...
$pathinfo = [Microsoft.VisualBasic.Interaction]::InputBox('Please Enter The Folder Name. ', 'Folder', "(EX.. M945-001 - Nobel)")
<#
#***** importing the csv file 
...
if($vaultserverarchive -like "wendt-vault"){
write-host "now connected to wendt-vault"
...
if($attatchedfile.'File Name'){
try{
$attatcherror = "none"
$updateall = Update-VaultFile -File $attatchedfile.'Parent File Path' -AddChild $attatchedfile.'File Path'
}catch{$attatcherror = "error attatching $attatchedfile.'File Name' to $attatchedfile.'Parent File' "}
$copyreport.rows.add($attatchedfile.'File Name',$attatcherror,"Add Attatchment")
}
}
...
$copyreport | export-csv -Path "C:\vault Data\Vault Reports\Other Reports\Copy Archive Report $date.csv" -NoTypeInformation
$result = [Autodesk.DataManagement.Client.Framework.Vault.Library]::ConnectionManager.LogOut($global:VaultConnection)
