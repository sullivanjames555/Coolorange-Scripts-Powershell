...
######## iNPUT OF FILE NAME ##### 
$vfile = [Microsoft.VisualBasic.Interaction]::InputBox('Please Enter The File Name', 'Assembly File', "(EX..800-123-1234.iam)")
$vfile1 = get-vaultfiles -properties @{'Name'="$vfile"}
[System.Windows.MessageBox]::Show('Starting saving and conversion of $vfile1. This process could take some time Click OK to continue')
$workingDirectory = "C:\Temp\Wendt"

###setting the variable for files ####
$fullpath = $vfile1._FullPath
$entitypath = $vfile1.Path

###### this saves the valt file and all necessary components to the Temp Folder on C Drive #############
$save = save-vaultfile -file $fullpath -DownloadDirectory "C:\Temp\" 
### $iamfile is necessary to select the single item and store it as a variable ##########
$iamfile = $save |where-object {$_.Name -eq $vfile1.Name}
$Dwfname = $iamfile.LocalPath + ".dwf" 
$movefile = $iamfile._Name + ".dwf"

#### these will be the new names #############
