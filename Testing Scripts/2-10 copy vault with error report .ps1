...
new-cooltable -Tablename "CopyReport" -columnnames "File Name", "error"
#**************************************************************************************************
#endregion
...
$copyreport.rows.add($subfile.'File Name', $suberror)
...
}catch{$iamerror = $error}
$copyreport.rows.add($iamfile.'File Name',$iamerror)
clear-variable iamerror
}
...
$copyreport.rows.add($childfile.'File name',$updateerror)
clear-variable updateerror
...
}catch{$attatcherror = $error}
$copyreport.rows.add($attatchedfile.'File Name',$attatcherror)
clear-variable attatcherror
}
...
$copyreport | export-csv -Path "C:\vault Data\Vault Reports\Other Reports\Copy Archive Report.csv" -NoTypeInformation
$result = [Autodesk.DataManagement.Client.Framework.Vault.Library]::ConnectionManager.LogOut($global:VaultConnection)
}
